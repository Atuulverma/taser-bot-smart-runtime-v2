...
try:
    _ENGINE_ORDER = [s.strip().lower() for s in (getattr(C, "ENGINE_ORDER", _def_order) or _def_order)]
...
def _trendscalp_is_only_engine(meta: dict) -> bool:
    ...
    only_ts = (_ENGINE_ORDER == ["trendscalp"]) or (len(_ENGINE_ORDER) == 1 and _ENGINE_ORDER[0] == "trendscalp")
...
def _dbg_meta_block(meta: dict, note: str = "") -> str:
    ...
    lines = ["\nDEBUG:", f"• note: {note}" if note else "• validators missing", f"• meta.keys: {', '.join(keys[:20])}{' …' if len(keys) > 20 else ''}", f"• state.keys: {', '.join(s_keys[:20])}{' …' if len(s_keys) > 20 else ''}", f"• cfg.keys: {', '.join(c_keys[:20])}{' …' if len(c_keys) > 20 else ''}"]
...
def _ensure_ts_meta(meta: dict, price: float = None) -> dict:
...
def _coalesce_state_cfg(meta: dict) -> tuple:
    ...
    if not isinstance(state, dict):
        state = m.get("validators") if isinstance(m.get("validators"), dict) else m.get("filters") if isinstance(m.get("filters"), dict) else None
    if not isinstance(cfg, dict):
        cfg = m.get("cfg") if isinstance(m.get("cfg"), dict) else m.get("config") if isinstance(m.get("config"), dict) else None
...
def _dbg_rx(func_name: str, price: float, meta: dict) -> str:
    """
...
def fmt_levels(meta):
    bits=[]
    if meta.get("pdh"): bits.append(f"PDH {meta['pdh']:.4f}")
    if meta.get("pdl"): bits.append(f"PDL {meta['pdl']:.4f}")
    if meta.get("vwap5"): bits.append(f"VWAP {meta['vwap5']:.4f}")
    if meta.get("avwap_hi"): bits.append(f"AVWAP↑ {meta['avwap_hi']:.4f}")
    if meta.get("avwap_lo"): bits.append(f"AVWAP↓ {meta['avwap_lo']:.4f}")
    return " | ".join(bits)
def fmt_validators_trendscalp(meta):
    ...
    e5 = state.get("ema200_5"); e15 = state.get("ema200_15")
    if e5 is not None:
        if state.get("ma_long_ok") is not None or state.get("ma_short_ok") is not None:
            parts.append("EMA200(5/15) aligned ✓" if (state.get("ma_long_ok") or state.get("ma_short_ok")) else "EMA200(5/15) misaligned ✗")
        else:
            # If no booleans, just acknowledge presence to avoid hiding context
...
def fmt_details_trendscalp(meta):
    ...
    atr = state.get("atr14_last"); floor = cfg.get("TS_VOL_FLOOR_PCT")
    adx = state.get("adx_last"); adx_min = cfg.get("TS_ADX_MIN")
    rsi15 = state.get("rsi15"); rsi_blk = state.get("rsi_block")
    r1 = []
    ...
    e5 = state.get("ema200_5"); e15 = state.get("ema200_15")
    tlw = state.get("tl_width"); mult = cfg.get("TS_TL_WIDTH_ATR_MULT")
    r2 = []
    if e5 is not None: r2.append(f"EMA200(5m) {float(e5):.4f}")
    if e15 is not None: r2.append(f"EMA200(15m) {float(e15):.4f}")
    if mult is not None:
        if not use_regime_filter:
            r2.append("Regime disabled")
        elif tlw is not None and atr is not None:
            r2.append(f"TLw {float(tlw):.4f} vs {float(mult):.2f}×ATR {float(mult)*float(atr):.4f}")
...
def fmt_validators(meta):
    ...
    if _trendscalp_is_only_engine(meta) or (meta and any(k in meta for k in ("filter_state","validators","filters"))):
        try:
...
def suggest_next_step_trendscalp(meta):
    ...
    if state.get("vol_ok") is False and cfg.get("TS_VOL_FLOOR_PCT") is not None and meta.get("price") and state.get("atr14_last") is not None:
        pct = (float(state["atr14_last"]) / max(1e-9, float(meta["price"]))) * 100.0
        need.append(f"• Volatility: ATR14(5m)/Price {pct:.2f}% < {float(cfg['TS_VOL_FLOOR_PCT'])*100:.2f}% — wait for ≥ threshold.")
    if state.get("adx_ok") is False and cfg.get("TS_ADX_MIN") is not None and state.get("adx_last") is not None:
        need.append(f"• Trend strength: ADX(5m) {float(state['adx_last']):.1f} < {float(cfg['TS_ADX_MIN']):.0f} — wait for ≥ threshold.")
    if use_regime_filter and state.get("regime_ok") is False and cfg.get("TS_TL_WIDTH_ATR_MULT") is not None:
        need.append(f"• Regime: TL channel width must be ≥ {float(cfg['TS_TL_WIDTH_ATR_MULT']):.2f}×ATR14(5m).")
    if (state.get("ma_long_ok") is False) and (state.get("ma_short_ok") is False):
        need.append("• Align with trend: price must be on the correct side of 200‑EMA (5m & 15m) for the trade side.")
    # Fallback guidance if nothing explicit is set
    if not need:
        need.append("• Wait for TrendScalp gates to pass (ATR/ADX/RSI15/EMA/Regime) and a TL break in the trade direction.")
    return "\n".join(need)
def suggest_next_step(price, meta):
    if _trendscalp_is_only_engine(meta):
        try:
            ...
            if meta is not None:
                meta = dict(meta); meta.setdefault("price", price)
            return suggest_next_step_trendscalp(meta)
        except Exception:
            pass
    pdh=meta.get("pdh"); vwap5=meta.get("vwap5"); avhi=meta.get("avwap_hi"); avlo=meta.get("avwap_lo")
    need=[]
    if pdh:
        if price <= pdh:
            need.append(f"• Long: breakout + hold > PDH {pdh:.4f} with Δ+ & OI↑; SL below PDH/VWAP reclaim.")
        else:
            need.append(f"• Short: rejection near PDH {pdh:.4f} with Δ− or OI↘; SL just above wick.")
    if vwap5: need.append(f"• VWAP: reclaim/lose VWAP {vwap5:.4f} with confirming orderflow.")
    if avhi: need.append(f"• Watch AVWAP↑ {avhi:.4f} for stop-run + fail (short) or clean reclaim (long).")
    if avlo: need.append(f"• Watch AVWAP↓ {avlo:.4f} for sweep + reclaim (long).")
    if not need: need=["• Wait for price to reach PDH/PDL/VWAP/AVWAP with Δ & OI confirmation."]
    return "\n".join(need)
def no_trade_message(price, reason, meta):
    ...
    details_str = fmt_details_trendscalp(m) if (_trendscalp_is_only_engine(m) or ("filter_state" in m)) else ""
...
def signal_message(sig):
    ...
    return (f"✅ {sig.side} APPROVED — {C.PAIR}\n"
            f"Engine: {m.get('engine','—')}\n"
            f"Entry {sig.entry:.4f} | SL {sig.sl:.4f} | TP {', '.join([f'{t:.4f}' for t in sig.tps])}\n"
            f"Reason: {sig.reason}\n"
            f"Levels: {fmt_levels(m)}\n"
            f"{validators_line}"
            f"{details_block}"
...
def extension_message(draft, price):
    return (f"📈 PROFIT EXTENSION — {C.PAIR}\n"
...
def manual_close_message(pair, exit_px, pnl, draft, price_now):
    return (f"🧑‍💻 MANUAL CLOSE DETECTED — {pair}\n"
            f"Engine: {(getattr(draft, 'meta', {}) or {}).get('engine','—')}\n"
            f"Exit {exit_px:.4f} | PnL {pnl:.2f}\n"
            f"We’ll wait for the next valid setup.\n"
            f"Next:\n{suggest_next_step(price_now, draft.meta)}\n"
            f"Context: {fmt_levels(_ensure_ts_meta(getattr(draft, 'meta', {}) or {}, price_now))} | {fmt_validators(_ensure_ts_meta(getattr(draft, 'meta', {}) or {}, price_now))}")
