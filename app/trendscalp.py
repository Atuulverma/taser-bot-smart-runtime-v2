...
Two-stage absolute profit locks exist but are **paused by default for TrendScalp** (see TRENDSCALP_PAUSE_ABS_LOCKS).
"""
from typing import Dict, List, Optional, Tuple
import math

# Import Signal from taser_rules; provide a local shim if unavailable to avoid init failures
from . import config as C
try:
    from .taser_rules import Signal  # type: ignore
except Exception:
    from dataclasses import dataclass
    from typing import List, Dict
    @dataclass
    class Signal:  # minimal shim used by the pipeline
        side: str
        entry: float
        sl: float
        tps: List[float]
...
def _sma(arr: List[float], n: int) -> List[float]:
    ...
    for i,x in enumerate(arr):
        s+=float(x)
        if i>=n: s-=float(arr[i-n])
        out.append(s/max(1,min(i+1,n)))
    return out

def _rsi(closes: List[float], n:int=14)->List[float]:
    n=max(1,int(n)); rsis=[]; up=0.0; dn=0.0
    for i in range(len(closes)):
        if i==0: rsis.append(50.0); continue
...
def _adx(highs: List[float], lows: List[float], closes: List[float], n:int=14)->List[float]:
    ...
    tr_ema=_ema(tr,n); p_ema=_ema(plus_dm,n); m_ema=_ema(minus_dm,n)
    out=[]
    for i in range(len(closes)):
...
def _wavetrend(hlc3: List[float], chlen:int=10, avg:int=11)->List[float]:
    esa=_ema(hlc3,chlen); d=[abs(hlc3[i]-esa[i]) for i in range(len(hlc3))]
    de=_ema(d,chlen); ci=[(hlc3[i]-esa[i])/max(1e-12,0.015*de[i]) for i in range(len(hlc3))]
    return _ema(ci,avg)
...
def _ann_predict(closes, highs, lows, k:int, max_back:int, feature_count:int) -> Tuple[int,float]:
    n=len(closes)
    if n<6: return 0,0.0
    feats=_feature_series(closes, highs, lows)
    keys=list(_DEF_FEATURES)[:max(2,min(5,int(feature_count)))]
    series=[feats[k] for k in keys]
    preds=[]; dists=[]; last_d=-1.0
    back=min(max_back, n-5); start=n-back
    for i in range(start, n-5):
        if i%4!=0: continue
        vec=[s[i] for s in series]; now=[s[-1] for s in series]
        d=_lorentz_distance(now, vec)
        if d>=last_d:
            last_d=d; dists.append(d)
...
def _trendlines(highs, lows, closes, length:int, method:str, mult:float):
    n=len(closes); ph=[None]*n; pl=[None]*n; L=max(1,int(length))
    for i in range(L, n-L):
        if all(highs[i]>=highs[i-d] for d in range(1,L+1)) and all(highs[i]>highs[i+d] for d in range(1,L+1)):
            ph[i]=highs[i]
        if all(lows[i]<=lows[i-d] for d in range(1,L+1)) and all(lows[i]<lows[i+d] for d in range(1,L+1)):
            pl[i]=lows[i]
    def slope_val(i):
        if method=="stdev":
            m=max(1,L); mean=sum(closes[i-m+1:i+1])/m
            var=sum((closes[j]-mean)**2 for j in range(i-m+1,i+1))/m
            sd=math.sqrt(var); return sd/max(1,m)*mult
        if method=="linreg":
            m=max(1,L); xs=list(range(i-m+1,i+1)); ys=closes[i-m+1:i+1]
            xbar=sum(xs)/m; ybar=sum(ys)/m
    ...
    upper=[0.0]*n; lower=[0.0]*n; s_ph=0.0; s_pl=0.0
    for i in range(n):
        if ph[i] is not None:
            s_ph=slope_val(i); upper[i]=ph[i]
        else:
            upper[i]=upper[i-1]-s_ph if i>0 else closes[0]
        if pl[i] is not None:
            s_pl=slope_val(i); lower[i]=pl[i]
        else:
    ...
    for i in range(n):
        ...
        upos[i] = 1 if cond_up and (i>0 and not ((ph[i-1] is None) and (closes[i-1] > upper[i-1] - s_ph * L))) else 0
        dnos[i] = 1 if cond_dn and (i>0 and not ((pl[i-1] is None) and (closes[i-1] < lower[i-1] + s_pl * L))) else 0
    return upper, lower, upos, dnos

# ---------- public: entry ----------
def scalp_signal(price: float,
                 tf5: Dict[str,List[float]], tf15: Dict[str,List[float]], tf1h: Dict[str,List[float]],
                 pdh: Optional[float], pdl: Optional[float],
                 oi_up: Optional[bool], delta_pos: Optional[bool],
    ...
    closes=tf5["close"]; highs=tf5["high"]; lows=tf5["low"]; vols=tf5.get("volume", [])
    ...
    try:
        ...
        curr_bar_ts = None
    need_bars = max(int(getattr(C, "TS_TL_LOOKBACK", 14)), int(getattr(C, "TS_EMA_SLOW", 20)), 20) + 10
    if len(closes) < need_bars or len(highs) < need_bars or len(lows) < need_bars:
        return Signal("NONE",0,0,[],"trendscalp: insufficient data",{"engine":"trendscalp","need":int(need_bars),"have":int(len(closes))})

    # --- RE-ENTRY GATES (TrendScalp-only; TASER unaffected) ---
    # A) Same-bar guard (reconfirm next 5m bar before retrying)
    if bool(getattr(C, "REQUIRE_NEW_BAR", True)) and _TS_LAST_ENTRY_BAR_TS is not None and curr_bar_ts is not None and _TS_LAST_ENTRY_BAR_TS == curr_bar_ts:
        try:
            telemetry.log("scan", "REENTRY_PRE", "same 5m bar (REQUIRE_NEW_BAR)", {"price": float(price), "side": "NONE", "last_entry": float(_TS_LAST_ENTRY_PX) if _TS_LAST_ENTRY_PX else None, "ago": 0})
        except Exception:
            pass
        return Signal("NONE", 0, 0, [], "trendscalp: same 5m bar", {"engine": "trendscalp"})

    # ML Lorentzian bias
    ml_sign, ml_sum = _ann_predict(closes, highs, lows, C.TS_NEIGHBORS, C.TS_MAX_BACK, C.TS_FEATURE_COUNT)
    ml_bias = "long" if ml_sign>0 else ("short" if ml_sign<0 else "neutral")

    # Trendlines breaks
    upper, lower, upos, dnos = _trendlines(highs, lows, closes, C.TS_TL_LOOKBACK, C.TS_TL_SLOPE_METHOD.lower(), C.TS_TL_SLOPE_MULT)
    upper_break = bool(upos[-1]); lower_break = bool(dnos[-1])

    # EMA trend & slope
    ema_fast=_ema(closes, int(getattr(C,"TS_EMA_FAST",8))); ema_slow=_ema(closes, int(getattr(C,"TS_EMA_SLOW",20)))
    def _s(arr, L): L=max(2,min(L,len(arr)-1)); return (arr[-1]-arr[-L])/max(1e-9,L)
    ema_up = ema_fast[-1] > ema_slow[-1] and _s(ema_fast, int(getattr(C,"TS_TREND_SLOPE_LEN",25))) >= float(getattr(C,"TS_TREND_SLOPE_MIN",0.0))
    ema_dn = ema_fast[-1] < ema_slow[-1] and _s(ema_fast, int(getattr(C,"TS_TREND_SLOPE_LEN",25))) <= -float(getattr(C,"TS_TREND_SLOPE_MIN",0.0))
    ...
    if _ma_require_15m:
        ...
        ma_long_ok  = (price >= max(_buf_long, (ema200_15 if ema200_15 is not None else -float("inf"))))
        ma_short_ok = (price <= min(_buf_short, (ema200_15 if ema200_15 is not None else float("inf"))))
    else:
    ...
    long_soft_ok  = use_soft and ma_long_ok  and (rsi15 is not None and rsi15 > 55.0) and (adx_last >= adx_soft_thr)
    short_soft_ok = use_soft and ma_short_ok and (rsi15 is not None and rsi15 < 45.0) and (adx_last >= adx_soft_thr)
    ...
    # [ADAPTIVE_REGIME][2025-09-10 08:34 IST] base assignment moved above; keep no-op for readability
    ...
    adapt_pb = max(base_pb, float(0.5 * atr_last_sig / max(1e-9, price)))  # opens tolerance during high vol
    near_fast = abs(price-ema_fast[-1])/max(1e-9,ema_fast[-1]) <= adapt_pb

    # WAI proxy from taser_rules (guarded)
    try:
        from .taser_rules import _wai_momentum  # type: ignore
    ...
    if bool(getattr(C,"TRENDSCALP_USE_AVOID_ZONES", False)):
        from .taser_rules import dynamic_avoid_zones, in_zones, vwap
        vwp=None
        try: vwp=vwap(highs,lows,closes,vols)[-1]
        except Exception: vwp=None
        zones, dbg = dynamic_avoid_zones(tf5, vwp, None, None)
        if in_zones(price, zones):
            return Signal("NONE",0,0,[],"trendscalp: avoid-zone",{"engine":"trendscalp","avoid":dbg})
        avoid_dbg=dbg
    ...
    long_ok  = pre_long_gate  and near_fast and (wai_long>=wai_min)  and not_bearish and ((ml_bias=="long"  and (upper_break or ema_up)) if require_both_long  else ((ml_bias=="long")  or upper_break or ema_up))
    short_ok = pre_short_gate and near_fast and (wai_short>=wai_min) and not_bullish and ((ml_bias=="short" and (lower_break or ema_dn)) if require_both_short else ((ml_bias=="short") or lower_break or ema_dn))
    ...
    if tentative_side and _TS_LAST_ENTRY_PX is not None and _TS_LAST_ENTRY_SIDE == tentative_side:
        ...
        if bars_ago is None or bars_ago <= _cool_bars:
            ...
            dist = abs(float(price) - float(_TS_LAST_ENTRY_PX)) / max(1e-9, float(_TS_LAST_ENTRY_PX))
            if dist < _dist_pct:
                try:
                    telemetry.log("scan", "REENTRY_BLOCK", "price too close to last entry (BLOCK_REENTRY_PCT)", {"price": float(price), "side": tentative_side})
                except Exception:
                    pass
                return Signal("NONE", 0, 0, [], "trendscalp: reentry distance block", {"engine": "trendscalp"})
    ...
    if not long_ok and not short_ok:
        try:
            telemetry.log_filter_block(
                ...
                filters=meta.get("filter_state", {}),
            )
        except Exception:
            pass
        return Signal("NONE", 0, 0, [], "trendscalp: filter block or no setup", meta)

    side = "LONG" if long_ok and not short_ok else ("SHORT" if short_ok and not long_ok else ("LONG" if ema_up else "SHORT"))

    # Record last entry context for re-entry gating on next scans
    if side in ("LONG", "SHORT"):
        _TS_LAST_ENTRY_PX = float(price)
        _TS_LAST_ENTRY_SIDE = side
        _TS_LAST_ENTRY_BAR_TS = curr_bar_ts if curr_bar_ts is not None else _TS_LAST_ENTRY_BAR_TS
    ...
    if str(getattr(C,"TS_STOP_MODE","trendline")).lower()=="trendline":
        ...
        vwp = None
        try:
            from .taser_rules import vwap as _vwap  # type: ignore
            vwp = _vwap(highs, lows, closes, vols)[-1]
        except Exception:
            vwp = None
        atr30 = _atr(highs, lows, closes, 30)[-1]
        try:
            from .taser_rules import _structural_sl  # type: ignore
    ...
    from .tp_calc import compute_tps
    atr30 = _atr(highs, lows, closes, 30)[-1]
    tps = compute_tps(price, sl, side, float(atr30), float(adx_last), C)

    reason=f"TrendScalp {ml_bias.upper()} {'UPBRK' if upper_break else ''}{'DNBRK' if lower_break else ''}{' EMAUP' if ema_up else (' EMADN' if ema_dn else '')}"
    return Signal(side, round(price,4), float(sl), tps, reason, meta)

# ---------- public: trendline trailing (separate from TASER manager) ----------
def scalp_manage(price: float,
                 side: str,
                 entry: float,
                 sl: float,
                 tps: List[float],
                 ...
                 meta: Dict) -> Dict:
    closes=tf5["close"]; highs=tf5["high"]; lows=tf5["low"]
    ...
    ema200_5 = float(_ema(closes, 200)[-1]) if len(closes) >= 200 else float(_ema(closes, min(200, len(closes)))[-1])
    adx_last = float(_adx(highs, lows, closes, 14)[-1])

    method=str(getattr(C,"TS_TL_SLOPE_METHOD","atr")).lower()
    L=int(getattr(C,"TS_TL_LOOKBACK",14)); mult=float(getattr(C,"TS_TL_SLOPE_MULT",1.0))
    upper, lower, upos, dnos = _trendlines(highs, lows, closes, L, method, mult)
    upper_now=float(upper[-1]); lower_now=float(lower[-1])

    ema_fast=_ema(closes, int(getattr(C,"TS_EMA_FAST",8)))
    ema_slow=_ema(closes, int(getattr(C,"TS_EMA_SLOW",20)))
    def _s(arr, n): n=max(2,min(n,len(arr)-1)); return (arr[-1]-arr[-n])/max(1e-9,n)
    ema_up = ema_fast[-1] > ema_slow[-1] and _s(ema_fast, int(getattr(C,"TS_TREND_SLOPE_LEN",25))) >= float(getattr(C,"TS_TREND_SLOPE_MIN",0.0))
    ema_dn = ema_fast[-1] < ema_slow[-1] and _s(ema_fast, int(getattr(C,"TS_TREND_SLOPE_LEN",25))) <= -float(getattr(C,"TS_TREND_SLOPE_MIN",0.0))
    ...
    fee = price*float(getattr(C,"FEE_PCT",0.0005))*float(getattr(C,"FEE_PAD_MULT",2.0))
    pad = max(0.6*atr_last, fee)
    min_pct=float(getattr(C,"MIN_SL_PCT",0.0045)); max_pct=float(getattr(C,"MAX_SL_PCT",0.0120))
    ...
    sl_min_step_atr = float(getattr(C, "TS_SL_MIN_STEP_ATR", 0.05))     # require >= 0.05*ATR step to move SL
    sl_min_buffer_atr = float(getattr(C, "TS_SL_MIN_BUFFER_ATR", 0.15)) # keep at least 0.15*ATR gap from price when tightening

    new_sl=float(sl); changed=False; exit_now=False; why=[]
    ...
    rev_min_r = float(getattr(C, "TS_REVERSAL_MIN_R", 0.15))  # require at least this move in R to allow a reverse
    ...
    confirm_n = int(getattr(C, "TS_EXIT_CONFIRM_BARS", 1))
    ...
    if str(side).upper()=="LONG":
        cand = lower_now - pad
        lo = entry - entry*max_pct; hi = entry - entry*min_pct
        cand = max(min(cand, hi), lo)
        if cand > new_sl: new_sl = cand; changed=True; why.append("trail lower TL")
        ...
        if move_from_entry >= give_arm_r * R0:
            gb_level = ref_price_long - give_frac * move_from_entry  # keep (1 - give_frac) of peak move
        ...
        if tl_break or ema_dn:
            ...
            context_ok = (adx_last >= rev_adx_min) and (px_ref <= ema200_5)  # only flip short if below 200-EMA(5m) and ADX strong
            if mr >= float(getattr(C, "TS_REVERSAL_MIN_R", 0.50)) and context_ok:
                try:
                    telemetry.log_reverse(
                        ...
                        tl_confirm_bars=int(getattr(C, "TS_EXIT_CONFIRM_BARS", 2)),
                    ...
                    pass
                exit_now = True
                why.append(f"reverse: TL/EMA down (confirmed) | moveR={mr:.2f}≥{float(getattr(C, 'TS_REVERSAL_MIN_R', 0.50)):.2f}, ADX={adx_last:.1f}≥{rev_adx_min}, 200EMA ok")
            else:
                try:
                    telemetry.log_reverse(
                        engine="trendscalp",
                    ...
                    pass
                why.append(f"no reverse: moveR={mr:.2f}, ADX={adx_last:.1f}, 200EMA test={(px_ref <= ema200_5)}")
    else:
        cand = upper_now + pad
        lo = entry + entry*min_pct; hi = entry + entry*max_pct
        cand = min(max(cand, lo), hi)
        if cand < new_sl: new_sl = cand; changed=True; why.append("trail upper TL")
        ...
        if move_from_entry >= arm_be_r * R0:
            ...
            if be_sl < new_sl:
                ...
                why.append(f"arm BE @{arm_be_r:.2f}R")
        if move_from_entry >= give_arm_r * R0:
            gb_level = ref_price_short + give_frac * move_from_entry  # keep (1 - give_frac) of peak move
            lo = entry + entry*min_pct; hi = entry + entry*max_pct
        ...
        if tl_break or ema_up:
            mr = _move_r(px_ref, entry, sl)
            context_ok = (adx_last >= rev_adx_min) and (px_ref >= ema200_5)  # only flip long if above 200-EMA(5m) and ADX strong
            if mr >= float(getattr(C, "TS_REVERSAL_MIN_R", 0.50)) and context_ok:
                try:
                    telemetry.log_reverse(
                        engine="trendscalp",
                    ...
                    pass
                exit_now = True
                why.append(f"reverse: TL/EMA up (confirmed) | moveR={mr:.2f}≥{float(getattr(C, 'TS_REVERSAL_MIN_R', 0.50)):.2f}, ADX={adx_last:.1f}≥{rev_adx_min}, 200EMA ok")
            else:
                try:
                    telemetry.log_reverse(
                        engine="trendscalp",
    ...
    return {"sl": float(round(new_sl,4)),
            "tps": final_tps,
