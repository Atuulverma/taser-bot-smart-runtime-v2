...
def abs_lock_from_entry(cur_sl: float, is_long: bool, entry: float, price: float, mfe_abs: float, abs_lock_usd: float) -> float:
    try:
...
def trail_fracR(cur_sl: float, is_long: bool, entry: float, tp: float, *, frac: float, atr_pad: float) -> float:
    base = float(entry) + float(frac) * (float(tp) - float(entry)) if is_long else float(entry) - float(frac) * (float(entry) - float(tp))
