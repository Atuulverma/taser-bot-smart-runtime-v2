...
from . import settings, state, telemetry
from .analytics import build_liquidity_heatmap
from .data import exchange, fetch_balance_quote, fetch_ohlcv, quote_from_pair
...
def _page_shell(body: str, title: str = "TASER Dashboard") -> str:
    ...
    <head>
      ...
      <style>
        ...
        .nav{{display:flex;gap:16px;padding:14px 18px;background:#121212;position:sticky;
             top:0;border-bottom:1px solid #222}}
        ...
        .pill{{padding:3px 8px;border-radius:12px;background:#1e1e1e;
              border:1px solid #333;color:#aaa;font-size:12px}}
        .ok{{color:#9fef00}} .warn{{color:#ffcc00}} .bad{{color:#ff5f5f}}
        input,button{{background:#111;border:1px solid #333;color:#ddd;
                    padding:6px 10px;border-radius:8px}}
        .grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(320px,1fr));gap:12px}}
        .card{{background:#101010;border:1px solid #222;border-radius:14px;padding:12px}}
        pre{{white-space:pre-wrap;word-break:break-word;background:#0f0f0f;
...
def api_overview():
    ...
    try:
        cur.execute(
            """SELECT id,symbol,side,entry,sl,tp1,tp2,tp3,qty,status,created_ts,
                       closed_ts,exit_price,realized_pnl
               FROM trades ORDER BY id DESC LIMIT 30"""
        )
        trades = cur.fetchall()
        cur.execute(
            """SELECT ts, tag, note FROM
    ...
    mode_txt = "PAPER" if C.DRY_RUN else "LIVE"
    html_parts.append(f"<div>Mode: <span class='pill'>{mode_txt}</span></div>")
    try:
        ...
        dr = True
        log = True
        scan = getattr(C, "SCAN_INTERVAL_SECONDS", 300)
    aut_txt = "ON" if aut else "OFF"
    html_parts.append(f"<div>Auto Trading: <span class='pill'>{aut_txt}</span></div>")
    dr_txt = "ON" if dr else "OFF"
    html_parts.append(f"<div>Paper (DRY_RUN): <span class='pill'>{dr_txt}</span></div>")
    ...
    last_price_txt = round(last_price, 4) if last_price else "—"
    html_parts.append(f"<div>Last Price: <b>{_escape(last_price_txt)}</b></div>")
    ws_status = _escape(st.get("ws_status", "—"))
    cvd_val = _escape(round(st.get("cvd") or 0, 2))
    html_parts.append(
        f"<div>Orderflow WS: <span class='pill'>{ws_status}</span> | CVD: <b>{cvd_val}</b></div>"
    )
    last_scan = st.get("last_scan_ts") or "—"
    html_parts.append(f"<div>Last Scan TS: <span class='pill'>{_escape(last_scan)}</span></div>")
    if st.get("last_audit"):
        last_audit_decision = st["last_audit"].get("decision")
        html_parts.append(
            f"<div>Last Audit: <span class='pill'>{_escape(last_audit_decision)}</span></div>"
        )
    if st.get("errors"):
    ...
    try:
        ...
        try:
            ...
            cur.execute(
                """SELECT id,symbol,side,entry,qty,status
                   FROM trades
        ...
        is_paper = getattr(C, "DRY_RUN", True)
        use_start = getattr(C, "PAPER_USE_START_BALANCE", False)
        if is_paper and use_start:
            paper_sizing = "START BAL"
        elif is_paper:
            paper_sizing = "EQUITY"
        else:
            paper_sizing = "LIVE"
        ...
        ccy = paper_start.get("ccy", quote_ccy)
        start_str = f"{start_amt:.2f} {ccy}"
        equity_str = f"{equity:.2f} {ccy}"
        realized_str = f"{realized_pnl_total:.2f}"
        unreal_str = f"{unreal:.2f}"
        html_parts.append(
            f"<div>Paper Sizing Mode: <span class='pill'>{_escape(paper_sizing)}</span></div>"
        )
        html_parts.append(f"<div>Start: <b>{_escape(start_str)}</b></div>")
        html_parts.append(
            "<div>Realized PnL: <b>"
            f"{_escape(realized_str)}</b> | Unrealized: <b>{_escape(unreal_str)}</b></div>"
        )
        html_parts.append(f"<div>Equity: <b>{_escape(equity_str)}</b></div>")
        _dd_cls = "bad" if dd_abs > 0 else "ok"
        _dd_txt = _escape(f"${dd_abs:.2f} ({dd_pct:.2f}%)")
        _rec_txt = _escape(f"{rec_pct:.2f}%")
        html_parts.append(
            "<div>Drawdown: "
            f"<b class='{_dd_cls}'>{_dd_txt}</b> | "
            f"Recovered: <b>{_rec_txt}</b></div>"
        )
        html_parts.append("<hr>")
        avg_pnl_txt = (
            f"{avg_daily_realized_pnl_val:.2f}" if avg_daily_realized_pnl_val is not None else "—"
        )
        html_parts.append(
            "<div><b>Avg Daily Realized PnL</b>: "
            f"<span id='avg-pnl-val'>{_escape(avg_pnl_txt)}</span></div>"
        )
        if days_to_breakeven is not None:
            brk_txt = f"{days_to_breakeven:.1f}"
            html_parts.append(f"<div>Days to Breakeven: <b>{_escape(brk_txt)}</b></div>")
        avg_val_str = (
            f"{avg_daily_realized_pnl_val:.2f}" if avg_daily_realized_pnl_val is not None else ""
        )
        html_parts.append(
            ...
            <form id='avg-pnl-form' style='margin-top:6px;display:flex;gap:6px;align-items:center'
                  onsubmit="event.preventDefault(); (async () => {{
                    ...
                    try {{
                      ...
                      if (j.ok) {{
                        document.getElementById('avg-pnl-val').innerText =
                ...
                <input id='avg-pnl-input' name='amount' type='number' step='0.01' min='0'
                       style='width:90px'
                       value='{avg_val_str}'>
        ...
        html_parts.append(
            "<div class='card' style='border-color:#553'><b>Recovery:</b> "
            "error computing snapshot</div>"
        )
    ...
    try:
        ...
        def _render_engine_table(rows):
            ...
            max_px = 260
            html = [
                "<table><tr><th>Engine</th><th>Trades</th><th>W/L</th>"
                "<th>Total PnL</th><th></th></tr>"
            ]
            for r in rows:
                eng = str(r[0] or "?")
                trd = int(r[1] or 0)
        ...
        html_parts.append(
            "<div id='engine-7d' style='display:none'>" + _render_engine_table(rows7) + "</div>"
        )
        html_parts.append(
            "<script>\n"
            "(function(){\n"
            ...
            "  if(b24) b24.onclick=()=>{ e24.style.display=''; "
            "e7.style.display='none'; };\n"
            "  if(b7) b7.onclick=()=>{ e7.style.display=''; "
        ...
        html_parts.append(
            "<div class='card' style='border-color:#553'><b>Engine PnL:</b> error computing</div>"
        )
    ...
    try:
        ...
        def _render_exchange_table(rows):
            if not rows:
                return "<div class='muted'>No closed trades in window.</div>"
            html = [
                "<table><tr><th>Exchange</th><th>Trades</th><th>W/L</th><th>Total PnL</th></tr>"
            ]
            for r in rows:
                exch = str(r[0] or "?")
                trd = int(r[1] or 0)
        ...
        html_parts.append(
            "<div id='exch-pnl-7d' style='display:none'>"
            + _render_exchange_table(rows_7d)
            + "</div>"
        )
        html_parts.append(
            "<script>\n"
        ...
        html_parts.append(
            "<div class='card' style='border-color:#553'><b>Exchange PnL:</b> error computing</div>"
        )
    ...
    if ls:
        html_parts.append(
            f"<div>Side: <b>{_escape(ls.get('side'))}</b> | "
            f"Entry: {_escape(ls.get('entry'))} | SL: {_escape(ls.get('sl'))}</div>"
        )
    ...
    # Trades table
    html_parts.append(
        "<div class='card' style='grid-column:1/-1'><h3>Recent Trades</h3>"
    ...
    for t in trades:
        ...
        html_parts.append(
            ...
            f"<td>{_escape(status)}</td><td>{_fmt_ts(cts)}</td>"
            f"<td>{_fmt_ts(ets) if ets else ''}</td>"
    ...
    # Events table
    html_parts.append(
        "<div class='card' style='grid-column:1/-1'><h3>Events</h3>"
        "<table><tr><th>TS</th><th>Tag</th><th>Note</th></tr>"
    )
    for ts, tag, note in events:
        html_parts.append(
            f"<tr><td>{_fmt_ts(ts)}</td><td>{_escape(tag)}</td><td>{_escape(note)}</td></tr>"
        )
    html_parts.append("</table></div>")
...
def reset_paper_start():
    ...
    try:
        ...
        return JSONResponse(
            {"ok": False, "msg": f"Wallet fetch error: {e}"},
...
def api_positions():
    ...
    try:
        ...
        live_quote = None
        banner = (
            "<div class='card' style='border-color:#553'><b>Note:</b> Wallet fetch error: "
            f"{str(e)}</div>"
        )
    ...
    try:
        ...
        if not banner:
            banner = (
                "<div class='card' style='border-color:#553'><b>Note:</b> "
                "fetch_positions unavailable or creds missing.</div>"
            )
    ...
    live_bal_html = (
        f"<b>{live_quote:.2f} {quote_ccy}</b>" if live_quote is not None else "<i>n/a</i>"
    )
...
def api_telemetry(component: str = "", q: str = "", limit: int = 200):
    ...
    html_table = (
        "<table><tr><th>TS</th><th>Component</th><th>Tag</th>"
        "<th>Message</th><th>Payload</th></tr>" + "".join(tr) + "</table>"
    )
...
def api_data():
    ...
    dump_1m = _escape(json.dumps(lastN(tf1m, 50), indent=2)[:4000])
    dump_5m = _escape(json.dumps(lastN(tf5, 80), indent=2)[:4000])
    dump_15 = _escape(json.dumps(lastN(tf15, 80), indent=2)[:4000])
    dump_1h = _escape(json.dumps(lastN(tf1h, 80), indent=2)[:4000])
    html_body = "<div class='grid'>"
    html_body += "<div class='card'><h3>1m (last 50)</h3><pre>" + dump_1m + "</pre></div>"
    html_body += "<div class='card'><h3>5m (last 80)</h3><pre>" + dump_5m + "</pre></div>"
    html_body += "<div class='card'><h3>15m (last 80)</h3><pre>" + dump_15 + "</pre></div>"
    html_body += "<div class='card'><h3>1h (last 80)</h3><pre>" + dump_1h + "</pre></div>"
    html_body += "</div>"
    return JSONResponse({"html": html_body})
...
def view_settings():
    ...
    chk_aut = "checked" if s_aut else ""
    chk_dry = "checked" if s_dry else ""
    chk_log = "checked" if s_log else ""
    body = f"""
    <form method='post' action='/settings' class='card'>
      <h3>Runtime Settings</h3>
      <label>Auto Trading:</label> <input type='checkbox' name='AUTO_TRADING' {chk_aut}><br>
      <label>Paper Trade (DRY_RUN):</label> <input type='checkbox' name='DRY_RUN' {chk_dry}><br>
      <label>Console Logging:</label> <input type='checkbox' name='CONSOLE_LOG' {chk_log}><br>
      <label>Scan Interval (sec):</label>
      <input type='number' name='SCAN_INTERVAL_SECONDS' value='{_escape(s_int)}'><br><br>
...
def save_settings(
    ...
    if prev_dry and not new_dry:
        if (CONFIRM or "").strip().upper() != "ARM LIVE":
            return HTMLResponse(
                _page_shell(
                    "<h3 style='color:#ff5f5f'>Confirmation text missing. "
                    "Type 'ARM LIVE' to go live.</h3>",
                    "Settings",
                )
            )
...
def export_trades_csv():
    ...
    try:
        ...
        con.close()
    header = (
        "id,symbol,side,entry,sl,tp1,tp2,tp3,qty,status,created_ts,closed_ts,"
        "exit_price,realized_pnl\n"
    )
    rows_text = "\n".join([",".join([str(x) for x in r]) for r in rows])
    out = header + rows_text
