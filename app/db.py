...
def init():
    con=sqlite3.connect(C.DB_PATH); cur=con.cursor()
...
def exec(sql, params=()):
    con=sqlite3.connect(C.DB_PATH); cur=con.cursor()
    cur.execute(sql, params); con.commit(); lid=cur.lastrowid; con.close(); return lid
def query(sql, params=()):
    con=sqlite3.connect(C.DB_PATH); cur=con.cursor()
    cur.execute(sql, params); rows=cur.fetchall(); con.close(); return rows
def new_trade(symbol, side, entry, sl, tps, qty, meta):
    now = int(time.time()*1000); tps = tps or []; tp1,tp2,tp3 = (tps+[None,None,None])[:3]
    # derive labels
    try:
    ...
    try:
        ...
        base_cols = ["symbol","side","entry","sl","tp1","tp2","tp3","qty","status","created_ts","closed_ts","exit_price","realized_pnl","meta_json"]
        base_vals = [symbol, side, entry, sl, tp1, tp2, tp3, qty, "OPEN", now, None, None, None, json.dumps(meta)]
        extra_cols, extra_vals = [], []
        if use_account:
            extra_cols.append("account"); extra_vals.append(account)
        if use_engine:
            extra_cols.append("engine"); extra_vals.append(engine)
        if use_exchange:
            extra_cols.append("exchange"); extra_vals.append(exchange)
        ...
        con.close()
    exec("INSERT INTO events(trade_id,ts,tag,note) VALUES(?,?,?,?)", (tid, now, "NEW_TRADE", f"{side} @ {entry} | SL {sl} | TPs {tps}"))
    return tid
def update_status(tid:int, status:str, note:str=""):
    now=int(time.time()*1000)
    exec("UPDATE trades SET status=? WHERE id=?", (status, tid))
    exec("INSERT INTO events(trade_id,ts,tag,note) VALUES(?,?,?,?)", (tid, now, status, note))
def close_trade(tid:int, exit_px:float, pnl:float, tag:str):
    now=int(time.time()*1000)
    exec("UPDATE trades SET status=?, closed_ts=?, exit_price=?, realized_pnl=? WHERE id=?",
         (tag, now, exit_px, pnl, tid))
    exec("INSERT INTO events(trade_id,ts,tag,note) VALUES(?,?,?,?)", (tid, now, "CLOSED", f"{tag} @ {exit_px}, PnL {pnl}"))
def add_order(tid:int, oid:str, typ:str, side:str, price:float, qty:float, status:str):
    now=int(time.time()*1000)
    exec("""INSERT INTO orders(trade_id,order_id,type,side,price,qty,status,created_ts)
            VALUES(?,?,?,?,?,?,?,?)""", (tid, oid, typ, side, price, qty, status, now))
...
def save_snapshot(ts_ms: int, pair: str, kind: str, payload: dict):
    ...
    try:
        con = sqlite3.connect(C.DB_PATH)
        cur = con.cursor()
        cur.execute("INSERT INTO market_snapshots(ts,pair,kind,payload_json) VALUES(?,?,?,?)",
...
def update_trade_status(trade_id:int, status:str, exit_price:float=None, pnl:float=None):
    con = sqlite3.connect(C.DB_PATH); cur = con.cursor()
    try:
        if status in ("CLOSED","STOPPED"):
            cur.execute("""UPDATE trades SET status=?, closed_ts=?, exit_price=?, realized_pnl=?
                           WHERE id=?""",
...
def append_event(trade_id:int, tag:str, note:str=""):
    con = sqlite3.connect(C.DB_PATH); cur = con.cursor()
    try:
        cur.execute("""INSERT INTO events(trade_id,ts,tag,note)
                       VALUES(?,?,?,?)""",
...
def save_partial_fill(trade_id:int, which_tp:str, px:float, qty_filled:float):
    ...
    con = sqlite3.connect(C.DB_PATH); cur = con.cursor()
    try:
        cur.execute("""CREATE TABLE IF NOT EXISTS partials(
                         id INTEGER PRIMARY KEY AUTOINCREMENT,
                         trade_id INTEGER, ts INTEGER,
                         leg TEXT, price REAL, qty REAL)""")
        cur.execute("""INSERT INTO partials(trade_id,ts,leg,price,qty)
                       VALUES(?,?,?,?,?)""",
...
def set_setting(key: str, value):
    con = sqlite3.connect(C.DB_PATH); cur = con.cursor()
    try:
        cur.execute("INSERT OR REPLACE INTO settings(key,value,ts) VALUES(?,?,?)",
...
# =========================
# Settings KV (for paper start, etc.)
# =========================
def init_settings():
    con = sqlite3.connect(C.DB_PATH); cur = con.cursor()
    cur.execute("""
...
def list_open_trades():
    con = sqlite3.connect(C.DB_PATH); cur = con.cursor()
    try:
        cur.execute("""SELECT id,symbol,side,entry,sl,tp1,tp2,tp3,qty,created_ts
                       FROM trades WHERE status IN ('OPEN','PARTIAL')
                       ORDER BY id ASC""")
        rows = cur.fetchall()
        out = []
        for r in rows:
            out.append({
                "id": r[0], "symbol": r[1], "side": r[2], "entry": float(r[3]),
                "sl": float(r[4]), "tp1": r[5], "tp2": r[6], "tp3": r[7],
                "qty": float(r[8]), "created_ts": int(r[9] or 0),
            })
        return out
    finally:
        con.close()
...
def get_trade_engine(trade_id: int) -> str:
    ...
    try:
        ...
        try:
            cur.execute("SELECT COALESCE(json_extract(meta_json,'$.engine'),'taser') FROM trades WHERE id=?", (trade_id,))
            r = cur.fetchone()
            if r and r[0] is not None:
                return str(r[0])
        except Exception:
...
def engine_split_pnl(hours: int = 24):
    ...
    try:
        ...
        cur.execute("PRAGMA table_info(trades)"); cols = [c[1] for c in cur.fetchall()]
...
def get_trade(trade_id: int):
    ...
    con = sqlite3.connect(C.DB_PATH); cur = con.cursor()
    try:
        cur.execute("""SELECT id,symbol,side,entry,sl,tp1,tp2,tp3,qty,status,created_ts,closed_ts,exit_price,realized_pnl,meta_json
...
def update_trade_qty_and_status(trade_id: int, new_qty: float, status: str = None):
    ...
    try:
        if status is None:
            ...
            cur.execute("UPDATE trades SET qty=?, status=? WHERE id=?", (float(new_qty), status, trade_id))
        con.commit()
    finally:
        con.close()


def reduce_trade_qty(trade_id: int, qty_filled: float, price: float = None, leg: str = "TP1_FILL") -> float:
    """
    ...
    try:
        ...
        cur.execute("SELECT COALESCE(qty,0.0), COALESCE(status,'OPEN') FROM trades WHERE id=?", (trade_id,))
        r = cur.fetchone()
        if not r:
        ...
        new_status = "PARTIAL" if new_qty > 0.0 else r[1]  # keep existing status if flat; manager will finalize
        cur.execute("UPDATE trades SET qty=?, status=? WHERE id=?", (new_qty, new_status, trade_id))
        # event log
        now = int(time.time()*1000)
        note = f"{leg}: filled {qty_filled:.6f} @ {price if price is not None else 'n/a'}, remain {new_qty:.6f}"
        cur.execute("INSERT INTO events(trade_id,ts,tag,note) VALUES(?,?,?,?)", (trade_id, now, leg, note))
        con.commit()
    finally:
        con.close()
    ...
    try:
        ...
        save_partial_fill(trade_id, leg, float(price) if price is not None else 0.0, float(qty_filled))
    except Exception:
        pass
