...
def save_multi(ts_ms: int, hm: Dict[str, Any]):
    ...
    for tf, payload in hm.items():
        try:
            cur.execute(
                "INSERT INTO heatmap_levels(ts,tf,payload_json) VALUES(?,?,?)",
                (
                    int(ts_ms),
                    str(tf),
...
def recent_levels(tf: str, limit: int = 30) -> List[Dict[str, Any]]:
    ...
    cur = con.cursor()
    cur.execute(
        ("SELECT ts,payload_json FROM heatmap_levels WHERE tf=? ORDER BY ts DESC LIMIT ?"),
        (tf, limit),
    )
...
def confluence_gate(
    ...
    if side.upper() == "LONG" and hits_above >= need_tfs:
        block = True
        why = f"near multi-TF resistance ({hits_above} TFs)"
    if side.upper() == "SHORT" and hits_below >= need_tfs:
        block = True
        why = f"near multi-TF support ({hits_below} TFs)"
    return {
        "near": near,
        "block": block,
        "why": why,
