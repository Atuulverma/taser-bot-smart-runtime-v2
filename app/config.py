...
OHLCV_BACKFILL_MIN     = int(os.getenv("OHLCV_BACKFILL_MIN", "300"))   # ensure at least this many bars exist for indicators
...
# Fees / cushion
# DEPRECATED: not used by current managers; kept for backward-compat. Prefer FEES_PCT_PAD for BE floor.
...
MODE_RALLY_TP_ATR_MULTS = _floats_csv("MODE_RALLY_TP_ATR_MULTS", "0.90,1.60,2.60")
 ...
 # TP locking / trail behavior (new style) — default is 'trail_fracR'. Legacy $0.25/$0.50 absolute locks are removed; BE floor handled via FEES_PCT_PAD.
# TP locking / trail behavior (new style) — default is 'trail_fracR'. Legacy $0.25/$0.50 absolute locks are removed; BE floor handled via FEES_PCT_PAD.
...
POST_TP1_SL_DELAY_BARS = int(os.getenv("POST_TP1_SL_DELAY_BARS", "3"))     # bars to wait after TP1 before tightening SL
BE_EPS_ATR_MULT        = float(os.getenv("BE_EPS_ATR_MULT", "0.10"))        # BE +/- eps cushion in ATR units at TP1
...
STALL_TP_EPS      = float(os.getenv("STALL_TP_EPS", "0.02"))
 ...
 # ---- SL/TP safety guards & rate limits (matches .env) ---- (SCALP_ABS_LOCK_USD applies to optional scalper only; TrendScalp ignores when TRENDSCALP_PAUSE_ABS_LOCKS=true)
SL_MIN_GAP_ATR_MULT     = float(os.getenv("SL_MIN_GAP_ATR_MULT", "0.35"))     # min SL gap from price in ATR units
SL_MIN_GAP_PCT          = float(os.getenv("SL_MIN_GAP_PCT", "0.0012"))        # fallback min gap (~0.12%)
LOCK_NEVER_WORSE_THAN_BE= _bool(os.getenv("LOCK_NEVER_WORSE_THAN_BE", "true")) # after TP1, never trail below BE after fees
FEES_PCT_PAD            = float(os.getenv("FEES_PCT_PAD", "0.0010"))          # round‑trip fees cushion for BE floor/ceiling
# Messaging/ops de-dupe tolerances & cooldowns (used by surveillance TP/SL gates)
SL_EPS                = float(os.getenv("SL_EPS", "0.0003"))     # SL change is ignored if within this abs delta
SL_MIN_INTERVAL_S     = float(os.getenv("SL_MIN_INTERVAL_S", "20"))
TP_EPS                = float(os.getenv("TP_EPS", "0.0003"))     # TP tuple change ignored if within this abs delta
TP_MIN_INTERVAL_S     = float(os.getenv("TP_MIN_INTERVAL_S", "30"))
# Rate-limit tiny SL nudges across engines and set absolute scalp lock
TS_MIN_SL_CHANGE_ABS   = float(os.getenv("TS_MIN_SL_CHANGE_ABS", "0.02"))   # ignore SL updates smaller than this ($)
# NOTE: Not used by TrendScalp; only the optional scalper reads these.
SCALP_ABS_LOCK_USD     = float(os.getenv("SCALP_ABS_LOCK_USD", "0.50"))    # min hard BE+ lock once in profit

 # TrendScalp SL step/buffer knobs
TS_SL_MIN_STEP_ATR      = float(os.getenv("TS_SL_MIN_STEP_ATR", "0.12"))   # minimum ATR step required to move SL
TS_SL_MIN_BUFFER_ATR    = float(os.getenv("TS_SL_MIN_BUFFER_ATR", "0.45"))  # buffer from structure/heatmap when tightening

SL_TIGHTEN_COOLDOWN_SEC = int(os.getenv("SL_TIGHTEN_COOLDOWN_SEC", "55"))     # rate-limit SL updates
TP_EXTEND_COOLDOWN_SEC  = int(os.getenv("TP_EXTEND_COOLDOWN_SEC", "55"))      # rate-limit TP extensions
TP_EXTEND_MIN_DELTA_ATR = float(os.getenv("TP_EXTEND_MIN_DELTA_ATR", "0.20")) # min ATR improvement to extend TP2/3

 # Legacy lock modes (used only if TP_LOCK_STYLE='to_tp1' or legacy helpers are called)
# NOTE: With default TP_LOCK_STYLE='trail_fracR', these are ignored; safe to leave for backward-compat.
...
REENTRY_COOLDOWN_BARS_5M = int(os.getenv("REENTRY_COOLDOWN_BARS_5M", os.getenv("TS_COOLDOWN_BARS", "1")))
...
DYN_AVOID_EFFECTIVE = DYN_AVOID_ENABLED  # set .env DYN_AVOID_ENABLED=false to rely on TrendScalp gates only
...
TP_HIT_CONFIRM_BARS  = int(os.getenv("TP_HIT_CONFIRM_BARS", "0"))   # 0 = touch, >0 = require N 1m closes beyond TP
...
TS_VOL_FLOOR_PCT         = float(os.getenv("TS_VOL_FLOOR_PCT", "0.0020"))  # ATR14(5m)/price >= 0.20%
...
TS_MA_REQUIRE_15M = _bool(os.getenv("TS_MA_REQUIRE_15M", "false"))   # if true, also require 15m EMA alignment
TS_MA_BUFFER_PCT  = float(os.getenv("TS_MA_BUFFER_PCT", "0.0015"))   # extra buffer vs EMA to accept entry (~0.15%)
...
print(
    ...
    f"MIN_SL_PCT={MIN_SL_PCT} | MAX_SL_PCT={MAX_SL_PCT} | TP_MODE={TP_MODE} | TP_R_MULTIS={TP_R_MULTIS}",
    file=sys.stderr
)
print(
    "[CONFIG] TP_MODE => "
    f"MODE={TP_MODE} | ATR_MULTS={[TP1_ATR_MULT, TP2_ATR_MULT, TP3_ATR_MULT]} | "
    f"ADAPT={MODE_ADAPT_ENABLED} | CHOP_THRESH={{'ATR%': MODE_CHOP_ATR_PCT_MAX, 'ADX': MODE_CHOP_ADX_MAX}} | "
    f"CHOP_MULTS={MODE_CHOP_TP_ATR_MULTS} | RALLY_MULTS={MODE_RALLY_TP_ATR_MULTS}",
    file=sys.stderr,
)
print(
    "[CONFIG] DASH => "
    f"DASH_ENGINE_SPLIT_ENABLED={DASH_ENGINE_SPLIT_ENABLED} | LOOKBACK={DASH_ENGINE_SPLIT_LOOKBACK} | CSV={DASH_ENGINE_SPLIT_CSV} | "
    f"ENGINE_ORDER={ENGINE_ORDER}",
    file=sys.stderr
)
print(
    "[CONFIG] GUARDS => "
    f"TS_MIN_SL_CHANGE_ABS={TS_MIN_SL_CHANGE_ABS} | SCALP_ABS_LOCK_USD={SCALP_ABS_LOCK_USD} | "
...
print(
    "[CONFIG] TS_FILTERS => "
    f"VOL={TS_USE_VOL_FILTER} | REGIME={TS_USE_REGIME_FILTER} | ADX={TS_USE_ADX_FILTER} | RSI={TS_USE_RSI_FILTER}",
    file=sys.stderr,
)
...
if not DRY_RUN:
    print("[CONFIG] Live mode: forcing PAPER_USE_START_BALANCE=false and LIVE_SIZING_USE_FREE_MARGIN=true", file=sys.stderr)

print(
    "[CONFIG] EXIT_POLICY => "
    f"POST_TP1_SL_DELAY_BARS={POST_TP1_SL_DELAY_BARS} | BE_EPS_ATR_MULT={BE_EPS_ATR_MULT} | "
    f"TRAIL_STYLE={TRAIL_STYLE} | CHAND={{'preTP2': (CHAND_N_PRE_TP2, CHAND_K_PRE_TP2), 'postTP2': (CHAND_N_POST_TP2, CHAND_K_POST_TP2), 'postTP3': (CHAND_N_POST_TP3, CHAND_K_POST_TP3)}} | "
    f"STALL={{'bars': {STALL_BARS}, 'nearATR': {STALL_NEAR_TP_ATR}, 'rsi': {STALL_RSI_CONFIRM}, 'eps': {STALL_TP_EPS}}}",
    file=sys.stderr,
)
print(
    "[CONFIG] DATA => "
    f"OHLCV_TIMEFRAME={OHLCV_TIMEFRAME} | FETCH_LIMIT={OHLCV_FETCH_LIMIT} | BACKFILL_MIN={OHLCV_BACKFILL_MIN}",
    file=sys.stderr
)
