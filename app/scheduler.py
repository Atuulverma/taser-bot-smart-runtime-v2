# app/scheduler.py
# Directional-first, scalp-fallback; aggression-aware heatmap gating;
# audit off by default (commented path kept).
import asyncio
import os
import time
import traceback
...
_TS_SCALP_IMPORT_OK = _TF_FOLLOW_IMPORT_OK = False
# TrendScalp engine (Lorentzian + Trendlines) — optional
try:
    from app.trendscalp import scalp_signal as _ts_scalp_signal

    ts_scalp_signal = _ts_scalp_signal
...
try:
    from app.trendfollow import follow_signal as _tf_follow_signal

    tf_follow_signal = _tf_follow_signal
...
# Type aliases for optional plugin functions
TSScalpFn = Callable[..., Any]
TFFollowFn = Callable[..., Any]
BuildHMFn = Callable[..., dict[str, Any]]
ConfluenceGateFn = Callable[[dict[str, Any], float, str, float, int, int], dict[str, Any]]
HmSaveFn = Callable[[int, dict[str, Any]], Any]
InitFn = Callable[[], Any]
PurgeFn = Callable[[], Any]

# Declare these so mypy doesn't treat them as Modules-only when try/except falls back
telemetry: Any
logger: Any
state: Any
...
# Heatmap infra
try:
    from app.analytics import build_liquidity_heatmap as _build_liquidity_heatmap
    from app.analytics import build_liquidity_heatmap_multi as _build_liquidity_heatmap_multi

    build_liquidity_heatmap: Optional[BuildHMFn] = _build_liquidity_heatmap
    build_liquidity_heatmap_multi: Optional[BuildHMFn] = _build_liquidity_heatmap_multi
except Exception:
    build_liquidity_heatmap = None
    build_liquidity_heatmap_multi = None

try:
    from app.heatmap_store import confluence_gate as _confluence_gate
    from app.heatmap_store import init as _hm_init
    from app.heatmap_store import purge_old as _hm_purge
    from app.heatmap_store import save_multi as _hm_save

    confluence_gate: ConfluenceGateFn = _confluence_gate
    hm_init: InitFn = _hm_init
    hm_purge: PurgeFn = _hm_purge
    hm_save: HmSaveFn = _hm_save
except Exception:

    def _no_confluence(
        hm_multi: dict[str, Any],
        price: float,
        side: str,
        tol_pct: float = 0.0,
        need_tfs: int = 0,
    ...
    confluence_gate = _no_confluence
    ...
    def hm_save(ts_ms: int, hm: dict[str, Any]) -> None:
        return None
...
try:
    ...
    class _T:
        ...
        def log_startup_engine_order(self, eo: list[str]) -> None:
            print("[INIT ORDER]", eo)

    class _L:
        def console(self, *a: Any, **k: Any) -> None:
            print(*a)

    class _S:
        _kv: dict[str, Any] = {}

        def set_k(self, k: str, v: Any) -> None:
            self._kv[k] = v
...
try:
    logger.console(
        f"[INIT] TrendScalp import: {'OK' if globals().get('_TS_SCALP_IMPORT_OK') else 'FAIL'}"
    )
except Exception:
    pass
...
try:
    logger.console(
        f"[INIT] TrendFollow import: {'OK' if globals().get('_TF_FOLLOW_IMPORT_OK') else 'FAIL'}"
    )
except Exception:
    pass
# ------------------------------------------
# Generic engine cooldown: if last two closed trades for engine ended at SL,
# pause for N minutes (per-engine knob)
# ------------------------------------------
...
def _engine_on_cooldown(engine: str) -> Optional[str]:
    try:
        ...
        if cool_min <= 0:
            return None
        rows = db.query(
            "SELECT engine, status, closed_ts FROM trades WHERE engine "
            "= ? AND closed_ts IS NOT NULL ORDER BY id DESC LIMIT 2",
            (engine,),
        )
    ...
    if tf_follow_signal is not None:
        ...
        if tf and getattr(tf, "side", "NONE") != "NONE":
            ...
            telemetry.log(
                ...
                tf.reason,
                {
                    "side": getattr(tf, "side", "?"),
                    "sl": getattr(tf, "sl", None),
                    "tps": getattr(tf, "tps", None),
                },
            )
            return tf
...
def _gate_reentry(now_ts: int, price: float, side: str) -> Optional[str]:
    ...
    if last and last.get("closed"):
        ...
        if ago < getattr(C, "MIN_REENTRY_SECONDS", 60):
            ...
            if (
                side_u in ("LONG", "SHORT")
                and last.get("side") == side_u
                and last.get("entry") is not None
            ):
...
async def recover_open_trades(ex):
    ...
    for tr in opens:
        ...
        if hit is not None:
            ...
            telemetry.log(
                "recover",
                "CLOSED_SL",
                f"trade {trade_id} closed on recovery",
    ...
    if recovered_any:
        ...
        if still:
            ...
            d = _Draft(
                ...
                "RECOVERED",
                {"engine": db.get_trade_engine(_id)},
            )
            return {
                "resume": True,
                "trade_id": _id,
    ...
    if to_resume:
        d = _Draft(
            ...
            "RESUME",
            {"engine": db.get_trade_engine(to_resume["id"])},
        )
        return {
            "resume": True,
            "trade_id": to_resume["id"],
            "draft": d,
            "qty": float(to_resume["qty"]),
        }
...
def _try_trendscalp(price, tf5, tf15, tf1h, pdh, pdl, oi_up, delta_pos, tf1m, hm_multi=None):
    ...
    telemetry.log(
        ...
        sc.reason,
        {
            "side": getattr(sc, "side", "?"),
            "sl": getattr(sc, "sl", None),
            "tps": getattr(sc, "tps", None),
        },
    )
    return sc
    ...
    try:
        ...
        if path is None:
            ...
            suffix = (
                ...
                else "7d" if window_seconds == 7 * 24 * 60 * 60 else f"{window_seconds}s"
            )
...
async def scan_once(ex):
    ...
    if re_block:
        ...
        telemetry.log(
            ...
            "REENTRY_PRE",
            re_block,
            {"price": price, "side": "NONE", "last_entry": last_entry, "ago": ago},
        )
        return None
    ...
    try:
            ...
            if draft.side != "NONE":
                ...
                if gate.get("block"):
                    ...
                    for eng in next_try:
                        if eng == "trendscalp":
                            alt = _try_trendscalp(
                                price, tf5, tf15, tf1h, pdh, pdl, oi_up, delta_pos, tf1m, hm_multi
                            )
                        elif eng == "trendfollow":
                            alt = _try_trendfollow(
                                price, tf5, tf15, tf1h, pdh, pdl, oi_up, delta_pos, tf1m, hm_multi
                            )
                        elif eng == "taser":
                            alt = _call_taser_signal(
                                price, tf5, tf15, tf1h, pdh, pdl, oi_up, delta_pos, tf1m
                            )
                        else:
                            alt = None
                    ...
                    if getattr(draft, "side", "NONE") == "NONE":
                        ...
                        msg = no_trade_message(
                            price,
                            f"Heatmap block: {gate.get('why', '')}",
                            meta_hm,
                        )
                        try:
                            await tg_send(msg)
        ...
        if build_liquidity_heatmap is not None and not meta.get("heatmap_levels"):
    ...
    if draft.side == "NONE":
        ...
        for eng in _engine_order():
            ...
            if eng == "trendscalp":
                alt = _try_trendscalp(
                    price, tf5, tf15, tf1h, pdh, pdl, oi_up, delta_pos, tf1m, hm_multi or {}
                )
            elif eng == "trendfollow":
                alt = _try_trendfollow(
                    price, tf5, tf15, tf1h, pdh, pdl, oi_up, delta_pos, tf1m, hm_multi or {}
                ...
                alt = None
            attempt_map[eng] = (
                "NONE" if (alt is None or getattr(alt, "side", "NONE") == "NONE") else "OK"
            )
            if alt is not None and getattr(alt, "side", "NONE") != "NONE":
                draft = alt
                break
    ...
    if prox and "BLOCK_REENTRY_PCT" in prox:
        ...
        telemetry.log(
            ...
            "REENTRY_BLOCK",
            prox,
            {"price": price, "side": draft.side, "last_entry": last_entry, "ago": ago},
        )
        return None
        ...
        if verdict.get("decision") != "APPROVE":
            try:
                ...
                block_msg = (
                    ...
                    f"Proposed: {draft.side} @ {draft.entry:.4f} | "
                    f"SL {draft.sl:.4f} | TPs {draft.tps}\n"
    ...
    telemetry.log(
        ...
        f"{draft.side} — {draft.reason}",
        {
            ...
            "sl": draft.sl,
            "tps": draft.tps,
            "engine": (getattr(draft, "meta", {}) or {}).get("engine", "trendscalp"),
        },
    )
    state.set_k(
        "last_signal",
    ...
    if not (_entry_ok and _sl_ok):
        telemetry.log(
            "exec",
            "SKIP_SIZE_NO_EDGE",
            "Non-actionable signal (entry/sl missing or zero)",
        ...
        try:
            ...
            if last_err_bar != now_ts:
                try:
                    ...
                    pass
                try:
                    state.set_k("last_size_error_bar", int(now_ts))
                except Exception:
                    pass
        except Exception:
            try:
    ...
    try:
        db.append_event(
            tid,
            "AUDIT",
            f"{verdict.get('decision', 'SKIPPED')} — {verdict.get('why', 'AI off')}",
        )
    except Exception:
        pass
    ...
    try:
        telemetry.log(
            "exec",
            "BRACKET_PLACE",
            "placing bracket with flow-aware meta",
            {
                "tid": tid,
                "side": draft.side,
                "entry": draft.entry,
...
async def run_scheduler():
    ...
    db.init()
    if hasattr(db, "init_settings"):
        db.init_settings()
    if hasattr(db, "ensure_trades_account_column"):
    ...
    try:
        if not getattr(C, "DRY_RUN", True) and getattr(C, "PAPER_USE_START_BALANCE", False):
            ...
            telemetry.log(
                "run",
                "LIVE_SIZING",
                "DRY_RUN=false → forcing PAPER_USE_START_BALANCE=False",
                {},
            )
    except Exception:
        pass
    ...
    if rec and rec.get("resume"):
        ...
        qty = rec["qty"]
        try:
            await tg_send(f"Resuming management of open trade #{tid} — {d.side} @ {d.entry:.4f}")
        except Exception:
            pass
        try:
    ...
    # Periodic engine-split CSV export (keeps dashboard fresh).
    # Allow disabling by setting ENGINE_SPLIT_EXPORT_SECS <= 0
    ...
    if export_interval_s <= 0:
        ...
        try:
            telemetry.log(
                "export",
                "ENGINE_SPLIT_EXPORT_OFF",
                "engine-split CSV auto-export disabled",
                {},
            )
        except Exception:
            pass
    ...
    while True:
        try:
            ...
            try:
                telemetry.log("run", "STOP", "scheduler cancelled", {})
            except Exception:
                pass
            raise
        except Exception as e:
